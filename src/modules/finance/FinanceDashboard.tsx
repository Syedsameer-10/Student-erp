...
const FinanceDashboard = () => {
  ...
  useEffect(() => {
    ...
    void fetchLevels().then(setLevels);
  }, [isAccountant]);
  ...
  const handleCollectFee = () => {
    showToast('Fee collection actions are now expected to be recorded in Supabase.');
  };
  ...
  const handleDownloadBill = (fee: FeeRecord) => {
    ...
    const breakdown = [
      ['Tuition Fees', formatCurrency((Number(fee.totalAmount) * 0.7).toFixed(2))],
      ['Library & Lab Fees', formatCurrency((Number(fee.totalAmount) * 0.15).toFixed(2))],
      ['Van & Transport Fees', formatCurrency((Number(fee.totalAmount) * 0.1).toFixed(2))],
      ['Extracurricular Activities', formatCurrency((Number(fee.totalAmount) * 0.05).toFixed(2))],
    ];
    ...
    autoTable(doc, {
      ...
      foot: [['Total Amount', formatCurrency(Number(fee.totalAmount).toFixed(2))]],
  ...
  const pendingAccounts = students.filter((student) => getStudentFeeStatus(student) === 'PENDING').length;
  const totalRevenue = feeRecords.reduce((sum, fee) => sum + Number(fee.paidAmount), 0);
  const totalPending = feeRecords.reduce((sum, fee) => sum + Number(fee.pendingAmount), 0);
  const paymentSuccessRate = feeRecords.length
    ? Math.round((feeRecords.filter((fee) => fee.status === 'Paid').length / feeRecords.length) * 100)
    : 0;
      ...
      <div className="space-y-6 lg:pb-12 h-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            ...
            { title: 'Pending Accounts', value: pendingAccounts || 'Live', icon: Clock, color: 'bg-rose-500' },
            ...
            <tbody>
              {feeRecords.map((fee) => (
                <tr key={fee.id} className="hover:bg-slate-50/80 transition-colors border-b border-slate-50 last:border-0 group">
                  ...
                  <td className="px-6 py-4">
                    <span className="font-extrabold text-slate-900 block text-base">{formatCurrency(Number(fee.totalAmount).toLocaleString())}</span>
              ...
              {feeRecords.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-6 py-10 text-center text-sm text-slate-500">
                    No fee records are available in Supabase yet.
                  </td>
                </tr>
              )}
            </tbody>
